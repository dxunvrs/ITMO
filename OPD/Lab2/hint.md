...
Обращение к памяти в основном 2 (в момент выборки команды и в момент выборки операнда)  
Исключения: LOOP - ещё одно во время выполнения(итого 3), SWAM(также 3), CALL(также 3),  
ST (обращения 2, но второе во время выполнения, цикла выборки операнда нет)  
...
11. Вызов подпрограммы CALL DXXX  
    - Цикл выборки команды:
        - IP->BR,AR      
        - BR+1->IP MEM(AR)->DR
        - DR->CR 
